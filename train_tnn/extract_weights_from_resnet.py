...
def get_conv_filter( ops, lyr_idx ):
    conv_op = [ op for op in ops if "conv_filter" in op.name and op.type == "VariableV2" and "block_" + str(lyr_idx) in op.name ][0]
...
if __name__ == "__main__":
    ...
    nu = [ 0.7 ] + [ args.nu_conv ]*3 + [ args.nu_dense ]*3
    for lyr_idx in range(1, 5):
        conv_filter = get_conv_filter( ops, lyr_idx )
        lyr_name = "block_" + str(lyr_idx)
        eta_r = decode_twn( sess, conv_filter, nu[lyr_idx-1], "resnet_conv_" + lyr_name + ".csv" )
